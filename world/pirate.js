...
handler('tick', function () {
  ...
  if (Math.random() * 30 < 1) {
    var room = pirate.getCurrentRoom(),
        exits = _.keys(room.exits),
        i = Math.floor(Math.random()*exits.length),
        exit = exits[i];
    room.broadcast('The Pirate sashays to the ' + exit + ' exit');
    pirate.execute('go ' + exit);
  }
  if (Math.random() * 60 < 1) {
    pirate.execute('say Arr!!');
  }
});


handler('playerTalk', function (player, message) {
  if (player.getCurrentRoom() === pirate.getCurrentRoom() && pirate !== player) {
    pirate.execute('say ' + translate(message));
  }
});
